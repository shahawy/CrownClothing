...
function PaymentForm() {
    ...
    const paymentHandler = async (e) => {
        ...
        if (!stripe || !elements) {  // to exit and don't complete the function if there is no stripe or elements
            return;
        }

        setIsProcessingPayment(true)

        const response = await fetch("/.netlify/functions/createPaymentIntent", {  // the first argument is the path of the serverless function in our folder
            method: "post",
            headers: {
  ...
  return (
    ...
    <h2>Credit Card Payment</h2>
      <CardElement />
      <Button disabled={isProcessingPayment} style={{backgroundColor: isProcessingPayment && "#2B2730"}} buttonType="inverted" buttonName="Pay Now" />
    </form>
    </div>
  )
}
